...
def _generate_amo_address_misaligned_tests(test_data: TestData) -> list[str]:
    ...
    addr_reg, check_reg, limit_reg, reg1, reg2, reg3, reg4, reg5, reg6 = test_data.int_regs.get_registers(9, exclude_regs=[0])
    ...
    for offset in range(32):
        ...
        lines.extend(
            [   
                ...
                "",
                f"    # Update scratch address to be misaligned based a0 argument",
...
def _generate_amo_access_fault_tests(test_data: TestData) -> list[str]:
    ...
    addr_reg, limit_reg, check_reg, reg1, reg2, reg3, reg4, reg5, reg6 = test_data.int_regs.get_registers(9, exclude_regs=[0])
    ...
    lines.extend(
        [
            test_data.add_testcase(f"amo_access_fault", coverpoint, covergroup),
            f"    li  x{limit_reg}, 0",
            f"    li  x{check_reg}, 1",
            f"    la      x{addr_reg}, scratch",
            "",
...
@add_priv_test_generator("ExceptionsZaamo", required_extensions=["I", "Zicsr", "Zaamo", "Sm"], march_extensions=["I", "Zicsr", "A", "Zaamo"])
