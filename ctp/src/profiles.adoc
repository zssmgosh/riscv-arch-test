...
* MC100 Microcontroller Profile (https://github.com/riscv/riscv-unified-db/releases/tag/v0.1.0[MC100-32-CRD.pdf]): Phase 1
...
* Although the RVI20 profile states that ecall causes a trap to the execution environment, it also says that Zicsr instructions are not supported indepenent of the Zicntr or F instructions, so there
...
|ZihintntlZca|Compressed Nontemporal Locality Hint|<<t-ZihintntlZca-normative-rules>>||||x|x
...
* Delegation of MEI/MTI/MSI to S-mode - "most platforms will choose not to make thse interrupts delegatable from M-mode to S-mode"
